...
而泛型委托`EventHandler<TEventArgs>`的区别在于，第二项参数可以不派生自`System.EventArgs`，而是根据泛型类型参数`TEventArgs`决定。这意味着，如果希望在引发事件时传入10种参数，你需要根据`TEventArgs`写一个类，在里面定义10项属性。引发事件时，实例化这个类，把构造出的对象传到第二项参数里。
